...
def create_engine_from_conf(config):
    ...
    if not config.has_section('database'):
        syncwerk_data_dir = os.environ['SYNCWERK_CONF_DIR']
        if syncwerk_data_dir:
            path = os.path.join(syncwerk_data_dir, 'syncwerk.db')
        else:
            logging.warning('SYNCWERK_CONF_DIR not set, can not load sqlite database.')
            return None
        db_url = "sqlite:///%s" % path
        logging.info('[objectstorage] database: sqlite3, path: %s', path)
        ...
        if backend == 'mysql':
            ...
            logging.warning('[objectstorage] database: mysql, name: %s', dbname)
        elif backend == 'oracle':
            if config.has_option('database', 'host'):
                host = config.get('database', 'host').lower()
            else:
            ...
            logging.info('[objectstorage] database: oracle, service_name: %s', service_name)
        else:
...
def init_db_session_class(config):
    ...
    try:
        ...
        raise RuntimeError("invalid syncwerk config.")
